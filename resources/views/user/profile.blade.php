...
@section('content')
        ...
        <div class="container">
            <div class="page-heading">
                ...
                <div class="breadcrumb-image wow fadeInUp" data-wow-delay=".4s">
                    <img src="assets/img/Account-rafiki.png" alt="img">
                </div>
            </div>
        </div>
    </div>
    <div class="container pt-5 my-5">
        <div class="row d-flex justify-content-center pt-5">
                    ...
                    <div class="col-4 border p-3 position-relative tab-btn" data-tab="tab2">
                        ...
                        <div style="border-bottom: solid 2px #5bc0de;" class="col-4 d-flex flex-column">
                            <span class="fs-1">{{ $orders->filter(function($order) {
                                return $order->hosting_plan_id && $order->subscription && $order->subscription->status == 'active';
                            })->count() }}</span>
                ...
                <div class="col-12 col-md-5 d-flex justify-content-center">
                    <div class="col-6 border p-3 position-relative tab-btn" data-tab="tab3">
                        ...
                        <div style="border-bottom: solid 2px #d9534f;" class="col-4 d-flex flex-column">
                            <span
                                class="fs-1">{{ $orders->filter(function ($order) {
                                        return $order->domain_plan_id != null &&
                                            $order->domain_subscription &&
                                            $order->domain_subscription->status == 'active';
                                    })->count() }}</span>
            ...
            <div class="tab-content">
                                        ...
                                        @if ($order->subscription->status == 'active' or $order->subscription->status == 'ended')
                                            <tr>
                                                ...
                                                <td class=" text-nowrap">
                                                    {{ $order->subscription->start_date->format('d/m/Y') }}</td>
                                                <td class=" text-nowrap">
                                                    {{ $order->subscription->end_date->format('d/m/Y') }}</td>
                                                ...
                                                <td class=" text-nowrap">
                                                    @if ($order->subscription->status == 'active')
                                                        <div class="btn btn-primary col-12">
                                                            <a href="{{ $order->subscription->cPanel_link }}" target="_blank"
                                                    ...
                                                    @else
                                                        <div class="btn btn-info col-12">
                                                            <a href="{{ route('cart', [$order->plan->id, $order->subscription->id]) }}"
                                                                class="text-light text-decoration-none">
                                                                Re New
                                                            </a>
                                                        </div>
                                    ...
                                    @if ($order->vps_plan_id)
                                        @if ($order->vps_subscription->status == 'active' or $order->vps_subscription->status == 'ended')
                                            <tr>
                                                ...
                                                <td class=" text-nowrap">
                                                    {{ $order->vps_subscription->start_date->format('d/m/Y') }}</td>
                                                <td class=" text-nowrap">
                                                    {{ $order->vps_subscription->end_date->format('d/m/Y') }}</td>
                                                ...
                                                <td class=" text-nowrap">
                                                    ...
                                                    @else
                                                        <div class="btn btn-info col-12">
                                                            <a href="{{ route('cart_domain', [$order->plan->id, $order->vps_subscription->id]) }}"
                                                                class="text-light text-decoration-none">
                                                                Re New
                                                            </a>
                                                        </div>
                                    ...
                                    @if ($order->domain_plan_id)
                                        @if ($order->domain_subscription->status == 'active' or $order->domain_subscription->status == 'ended')
                                            <tr>
                                                ...
                                                <td class=" text-nowrap">
                                                    {{ $order->domain_subscription->start_date->format('d/m/Y') }}</td>
                                                <td class=" text-nowrap">
                                                    {{ $order->domain_subscription->end_date->format('d/m/Y') }}</td>
                                                <td class=" text-nowrap">
                                                    @if ($order->domain_subscription->status == 'active')
                                                        <div class="btn btn-success d-block col-12">active</div>
                                                    @else
                                                        <div class="btn btn-info col-12">
                                                            <a href="{{ route('domain_cart', [$order->domain_plan->id, $order->domain_subscription->id, $order->product]) }}"
                                                                class="text-light text-decoration-none">
                                                                Re New
                                                            </a>
                                                        </div>
                                                ...
                                                <td class=" text-nowrap">
                                                    <div class="btn btn-primary d-block update-dns" type="button"
                                                        data-toggle="modal" data-target="#activeDModal"
                                                        data-id="{{ $order->domain_subscription->id }}"
                                                        data-dns1="{{ $order->domain_subscription->dns1 }}"
                                                        data-dns2="{{ $order->domain_subscription->dns2 }}"
                                                        data-dns3="{{ $order->domain_subscription->dns3 }}"
                                                        data-dns4="{{ $order->domain_subscription->dns4 }}">update dns
                                                    </div>
                                                </td>
                                            </tr>
                                        @endif
                ...
                <div id="tab4" class="tab-pane  mt-5 col-12 boxshadow p-3" style="display:none;">
                    ...
                    <div class="table-responsive col-12">
                        <table class="table mt-3 table-bordered">
                            ...
                            <tbody>
                                @foreach ($orders as $order)
                                @if (
                                    ($order->domain_subscription && $order->domain_subscription->status == 'pending') ||
                                        ($order->plan && $order->subscription->status == 'pending') ||
                                ...
                                @foreach ($orders as $order)
                                    <tr>
                                        @if ($order->plan)
                                            ...
                                            <td class=" text-nowrap">
                                                {{ $order->subscription->start_date->format('d/m/Y') }}</td>
                                            <td class=" text-nowrap">{{ $order->subscription->end_date->format('d/m/Y') }}
                                            </td>
                                            <td class=" text-nowrap">
                                                <div
                                                    class="btn
                                         @if ($order->subscription->status == 'active') btn-success
                                        ...
                                        @elseif($order->vps_plan_id)
                                            ...
                                            <td class=" text-nowrap">
                                                <div
                                                    class="btn
                                         @if ($order->vps_subscription->status == 'active') btn-success
                                        ...
                                        @elseif($order->domain_plan_id)
                                            ...
                                            <td class=" text-nowrap">
                                                <div
                                                    class="btn
                                         @if ($order->domain_subscription->status == 'active') btn-success
    ...
    <div class="modal fade" id="activeDModal" tabindex="-1" role="dialog" aria-labelledby="activeDModalLabel"
        aria-hidden="true">
