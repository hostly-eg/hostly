                        ...
                        <div class="card">
                            <div class="card-header d-flex justify-content-between">
                                <div class="header-title">
                                    <h4 class="card-title">Affilates</h4>
                                </div>
                            </div>
                                    ...
                                    <table id="affilate-fields-table" class="table table-striped" role="grid" data-bs-toggle="data-table">
                                        ...
                                        <tbody>
                                            @foreach($affilates as $affilate)
                                                <tr>
                                                    <td>{{ $affilate->name }}</td>
                                                    <td>{{ $affilate->price }}</td>
                                                    <td>{{ $affilate->description }}</td>
                                                    <td>
                                                        @foreach($affilate->fields_rendered as $field)
                                                            <p>{{ $field['name'] }} : {{ $field['value'] }}</p>
                                                        @endforeach
                                                    </td>
                                                    <td>
                                                        <div class="d-flex align-items-center">
                                                            <a class="btn btn-sm btn-icon btn-warning" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit" href="{{ route('edit_affilate', $affilate->id) }}">
                                                            ...
                                                            <a class="btn btn-sm btn-icon btn-danger" data-bs-toggle="tooltip" data-bs-placement="top" title="Delete" onclick="showDeleteConfirmation('{{ route('delete_affilate', $affilate->id) }}')">
                                                                <span class="btn-inner">
