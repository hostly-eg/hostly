    ...
    <main class="main-content">
        ...
        <div class="conatiner-fluid content-inner mt-n5 py-0">
            <div class="card">
                ...
                @endif
                <div class="card-body">
                    <form class="row" action="{{route('update_affilate_field',$affilateField->id)}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                            <div class="form-group col-md-6">
                                <label class="form-label" for="name">Field Name</label>
                                <input type="text" name="name" id="name" class="form-control" value="{{$affilateField->name}}">
                            </div>
                            <div class="form-group col-md-6">
                                <label class="form-label" for="type">Field Type</label>
                                <select name="type" id="type" class="form-control">
                                    <option value="" disabled selected>Select Field Type</option>
                                    <option value="text" {{ $affilateField->type == 'text' ? 'selected' : '' }}>Text</option>
                                    <option value="boolean" {{ $affilateField->type == 'boolean' ? 'selected' : '' }}>Boolean</option>
                                </select>
                            </div>
                            <div class="form-group col-md-6">
                                <label class="form-label" for="end_prefix">End Prefix</label>
                                <input type="text" name="end_prefix" id="end_prefix" class="form-control" value="{{$affilateField->end_prefix}}">
                            </div>
                        </div>
