            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between">
                    <div class="header-title">
                        <h4 class="card-title">Edit {{$vps->name}} Vps Plan</h4>
                    </div>
                </div>
                ...
                @endif
                <div class="card-body">
                    <form class="row" action="{{route('update_vps',$vps->id)}}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="form-group col-md-6 col-12">
                            <label class="form-label" for="plan_name">Plan Name</label>
                            <input type="text" class="form-control" value="{{$vps->name}}" id="plan_name" name="plan_name">
                            @error('plan_name')
                        ...
                        <div class="form-group col-md-6 col-12">
                            ...
                            <input type="text" class="form-control" value="{{$vps->description}}" id="plan_description" name="plan_description">
                        ...
                        <div class="form-group col-md-6 col-12">
                            ...
                            <input type="text" class="form-control" value="{{$vps->price_1_month}}" id="price_month" name="price_month">
                            @error('price_month')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="form-group col-md-6 col-12">
                            ...
                            <input type="text" class="form-control" value="{{$vps->price_1_year}}" id="price_12month" name="price_12month">
                            @error('price_12month')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="form-group col-md-6 col-12">
                            ...
                            <input type="text" class="form-control" value="{{$vps->price_2_years}}" id="price_24month" name="price_24month">
                            @error('price_24month')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="form-group col-md-6 col-12">
                            ...
                            <input type="text" class="form-control" value="{{$vps->price_3_years}}" id="price_48month" name="price_48month">
                            @error('price_48month')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="form-group col-md-6 col-12">
                            ...
                            <input type="text" class="form-control" value="{{$vps->price_reNew}}" id="plan_Renew" name="plan_Renew">
                            @error('plan_Renew')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        <?php
                        $includeFeatures = json_decode($vps->features_included, true);
                        $not_includeFeatures = json_decode($vps->features_not_included, true);
                        ?>
                        <div class="form-group ">
                            ...
                            <div id="includeFeaturesList" class="features-list   mt-2">
                                @foreach($includeFeatures as $feature)
                                    <div class="feature-item d-flex align-items-center">
                                        <span class="feature-text">{{$feature}}</span>
                                        <span class="delete-btn" onclick="deleteIncludeItem('{{ $feature }}', this)">Delete</span>
                                    </div>
                                @endforeach
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <div id="excludeFeaturesList" class="features-list   mt-2">
                                @foreach($not_includeFeatures as $feature)
                                    <div class="feature-item d-flex align-items-center">
                                        <span class="feature-text">{{$feature}}</span>
                                        <span class="delete-btn" onclick="deleteItem('{{ $feature }}', this)">Delete</span>
                                    </div>
                                @endforeach
                            </div>
...
@section('script')
    <script>
        ...
        let includeFeatures = @json($includeFeatures);
        ...
        function deleteIncludeItem(feature, element) {
            includeFeatures = includeFeatures.filter(item => item !== feature);
            includeFeaturesHiddenInput.value = JSON.stringify(includeFeatures);
        ...
        let not_includeFeatures = @json($not_includeFeatures);
        ...
        function deleteItem(feature, element) {
            not_includeFeatures = not_includeFeatures.filter(item => item !== feature);
            not_includeFeaturesHiddenInput.value = JSON.stringify(not_includeFeatures);
            element.parentElement.remove();
        }
